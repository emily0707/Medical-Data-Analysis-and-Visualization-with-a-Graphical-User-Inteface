...
public class AddBeadsPageController implements Initializable {
    ...
    private void updateBeadsforExperiment(ActionEvent event) {
        int beadPlate = ModelForExperiments.getInstance().getCurrentPate();
        int curExperiment = ModelForExperiments.getInstance().getCurrentExperiment();
        ModelForExperiments.getInstance().setProbes(curExperiment, beadPlate, slectedBeads);
